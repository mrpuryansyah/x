...
public class StepPagerStrip extends View {
    ...
    private final Paint textPaintNext;
    ...
    public StepPagerStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        mPrevTabPaint.setAntiAlias(true);
        mPrevTabPaint.setColor(res.getColor(R.color.md_teal_300));

        mSelectedTabPaint = new Paint();
        mSelectedTabPaint.setAntiAlias(true);
        mSelectedTabPaint.setColor(res.getColor(R.color.md_light_blue_600));

        mSelectedLastTabPaint = new Paint();
        mSelectedLastTabPaint.setAntiAlias(true);
        mSelectedLastTabPaint.setColor(res.getColor(R.color.step_pager_selected_last_tab_color));

        mNextTabPaint = new Paint();
        mNextTabPaint.setAntiAlias(true);
        mNextTabPaint.setColor(res.getColor(R.color.md_grey_200));

        textPaint = new Paint();
        textPaint.setAntiAlias(true);
        textPaint.setColor(Color.WHITE);
        textPaint.setTextSize(res.getDimensionPixelSize(R.dimen.v6_akta_step_pager_text_size));
        textPaint.setTextAlign(Paint.Align.CENTER);

        textPaintNext = new Paint();
        textPaintNext.setAntiAlias(true);
        textPaintNext.setColor(res.getColor(R.color.md_grey_500));
        textPaintNext.setTextSize(res.getDimensionPixelSize(R.dimen.v6_akta_step_pager_text_size));
        textPaintNext.setTextAlign(Paint.Align.CENTER);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float totalWidth = (mPageCount-1) * (mTabWidth + mTabSpacing) - mTabSpacing;
        float totalLeft;
        ...
        for (int i = 0; i < mPageCount; i++) {
            ...
            canvas.drawCircle(mTempRectF.left,mTempRectF.top ,30,i < mCurrentPage
                    ? mPrevTabPaint
                    : (i > mCurrentPage
                    ? mNextTabPaint
                    : (i == mPageCount - 1
                    ? mSelectedLastTabPaint
                    : mSelectedTabPaint)));
            canvas.drawText(String.valueOf(i+1),mTempRectF.left,getHeight()/2 + convertDpToPixel(5),i < mCurrentPage
                    ? textPaint
                    : (i > mCurrentPage
                    ? textPaintNext
                    : (i == mPageCount - 1
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        setMeasuredDimension(
                ...
                View.resolveSize(
                        (int) mTabHeight
                                + getPaddingTop() + getPaddingBottom() + (int) convertDpToPixel(13),
                        heightMeasureSpec));
    }
